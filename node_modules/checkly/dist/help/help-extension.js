...
class ChecklyHelpClass extends core_1.Help {
    ...
    showRootHelp() {
        ...
        const hasConfig = ['checkly.config.ts', 'checkly.config.js', 'checkly.config.mts', 'checkly.config.mjs']
            .some(name => (0, fs_1.existsSync)((0, path_1.join)(cwd, name)));
        if (!hasConfig) {
            this.log('');
  ...
  CHECKLY_API_KEY       Checkly User API Key.
    `));
        const skillsCommands = this.renderList(skills_1.skills.map(s => [s.command, s.description]), {
            spacer: '\n',
            stripAnsi: this.opts.stripAnsi,
            indentation: 2,
        });
        this.log(this.section('DOCS & AGENT SKILLS', skills_1.skillsDescription + '\n' + skillsCommands));
        this.log('');
